...
public class CategoryGUI implements Reloadable {
    ...
    public void displayRankPointsGUI(ItemStack item, Player player, int requestedPage) {
        String categoryName = "Custom";
        ItemMeta meta = item.getItemMeta();

        meta.setDisplayName(translateColorCodes("&8\u2618 &lRank Requirements \u2618"));

        item.setItemMeta(meta);
        List<String> achievementPaths = getSortedMultipleAchievementPaths(categoryName);
        MultipleAchievements category = MultipleAchievements.getByName(StringUtils.substringBefore("Custom", "."));
        Map<String, Long> subcategoriesToStatistics = getMultipleStatisticsMapping(category, player);
        displayPage(categoryName, player, subcategoriesToStatistics, requestedPage, item, achievementPaths);
    }
	/**
	...
	private void displayPage(String categoryName, Player player, Map<String, Long> subcategoriesToStatistics,
		...
		if (categoryName.equals("Custom"))
            inventory = Bukkit.createInventory(inventoryHolder, guiSize, translateColorCodes("&2&lRank Requirements"));
